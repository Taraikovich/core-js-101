...
function chainPromises(array, action) {
  ...
  const processPromise = (promise) => promise.then((value) => {
    result = result !== undefined ? action(result, value) : value;
    return result;
  });

  array.forEach((promise) => {
    resultPromise = resultPromise.then(() => processPromise(promise)).catch(() => result);
  });
